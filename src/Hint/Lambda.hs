...
lambdaDecl
    o@(L loc1 (ValD _
        origBind@FunBind {fun_matches =
            MG {mg_alts =
                L _ [L _ (Match _ ctxt pats _ (GRHSs _ [L loc2 (GRHS _ [] origBody)] bind))]}}))
    ...
    where reform :: [LPat GhcPs] -> LHsExpr GhcPs -> LHsDecl GhcPs
          reform ps b = L loc $ ValD noExt $
            origBind
              {fun_matches = MG noExt (noLoc [noLoc $ Match noExt ctxt ps Nothing $ GRHSs noExt [noLoc $ GRHS noExt [] b] $ noLoc $ EmptyLocalBinds noExt]) Generated}
...
lambdaExp _ o@(SimpleLambda [LL _ (view' -> PVar_' x)] (L _ expr)) =
    case expr of
...
munge ident (LL ploc p) = LL ploc (VarPat noExt (L ploc $ mkRdrUnqual $ mkVarOcc [ident]))
munge _ x = x -- "{-# COMPLETE L #-}"
