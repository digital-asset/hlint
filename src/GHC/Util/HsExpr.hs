...
lambda vs body = noLoc $ HsLam noExt (MG noExt (noLoc [noLoc $ Match noExt LambdaExpr vs Nothing (GRHSs noExt [noLoc $ GRHS noExt [] body] (noLoc $ EmptyLocalBinds noExt))]) Generated)
...
simplifyExp' e@(L _ (HsLet _ (L _ (HsValBinds _ (ValBinds _ binds []))) z)) =
  ...
  case bagToList binds of
    [L _ (FunBind _ _(MG _ (L _ [L _ (Match _(FunRhs (L _ x) _ _) [] _rhs_sig (GRHSs _[L _ (GRHS _ [] y)] (L _ (EmptyLocalBinds _))))]) _) _ _)]
...
niceLambdaR' ss e =
  let grhs = noLoc $ GRHS noExt [] e :: LGRHS GhcPs (LHsExpr GhcPs)
      ...
      match = noLoc $ Match {m_ext=noExt, m_ctxt=LambdaExpr, m_pats=map strToPat ss,  m_rhs_sig=Nothing, m_grhss=grhss} :: LMatch GhcPs (LHsExpr GhcPs)
...
replaceBranches' (L s (HsCase _ a (MG _ (L l bs) FromSource))) =
  ...
  where
    ...
    f (L _ (Match _ CaseAlt _ _ (GRHSs _ xs _))) = [x | (L _ (GRHS _ _ x)) <- xs]
    ...
    g (L s1 (Match _ CaseAlt a sig (GRHSs _ ns b)) : rest) xs =
      cL s1 (Match noExt CaseAlt a sig (GRHSs noExt [cL a (GRHS noExt gs x) | (L a (GRHS _ gs _), x) <- zip ns as] b)) : g rest bs
